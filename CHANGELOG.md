...
### Rules — Added
- Document component-declared Facelet tags (`@FacesComponent(createTag = true)`) and their specification-defined default namespaces (`http://xmlns.jcp.org/jsf/component` in JSF 2.2–3.0 and `jakarta.faces.component` in Faces 4.0+), preventing false-positive reports of unknown or invalid XML namespaces in `faces-review`.
...
### Rules — Fixed
- `faces-review`: do not report custom XML namespaces as invalid without checking for `@FacesComponent(createTag = true)` classes, `*.taglib.xml` descriptors, composite libraries, and third-party dependencies. A declared namespace unused in a view is reported as `info` (unused import), never as `warning` or `error`.
- `faces-migrate`: add `http://xmlns.jcp.org/jsf/component` → `jakarta.faces.component` to the Faces 3.0 → 4.0 namespace migration list.
