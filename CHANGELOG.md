...
- The **Dialogs** section in `primefaces.md` is renamed **Dialogs and Overlays**, since what it now says depends on the overlay family, and it no longer claims that PrimeFaces appends `<p:dialog>` to the end of the `<body>`, nor that a dialog must always carry its own `UIForm`. Verified against PrimeFaces 8 through 16: `DialogRenderer` emits the widget's `appendTo` only when the attribute is set, `registerDynamicOverlay()` moves the overlay only when `cfg.appendTo` is present, and `DynamicOverlayWidget` explicitly exempts `Dialog` — subclasses included, the check being a lazy `instanceof` — from the auto-`@(body)` fallback that other overlays get, so `implicitDefaultValue = "@(body)"` on `DialogBase` never takes effect. Only the modal mask is always appended to `<body>`.
...
- What a body-appended dialog actually does on submit: PrimeFaces resolves a command's form server-side from the component tree and emits it as `f:`, so the action runs, but the request body is serialized from that form element in the DOM and the dialog's values are silently dropped — unless `partialSubmit="true"` serializes them from the `process` components instead, which is why reports of this symptom differ.
- The "similar overlay components" generalization is replaced by what each family actually does: auto-`@(body)` only inside a `.ui-dialog` for `<p:overlayPanel>`, `<p:confirmPopup>`, `<p:cascadeSelect>`, `<p:sidebar>` and the panel-based inputs `<p:selectOneMenu>`, `<p:selectCheckboxMenu>`, `<p:autoComplete>`, `<p:datePicker>`; unconditional `@(body)` for `<p:menu overlay="true">`, `<p:tieredMenu overlay="true">` and `<p:slideMenu overlay="true">` via `BaseMenu.initOverlay()`; and never for `<p:contextMenu>`, which skips that handling on purpose.
